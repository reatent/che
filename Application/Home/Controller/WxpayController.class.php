...
class WxpayController extends Controller {
    ...
    public function index($id,$type){
		...
		if($type=="by"){
		$by = M("by");
		$info=$by->where("ybid='$id'")->find();}
		elseif($type=="bj"){
			$by = M("ybj");
			$info=$by->where("ybid='$id'")->find();
		}
		$_SESSION['rands']=$info['rands'];
		$moeny = $info['price'];
		$tal_fee = $moeny * 100;
		//①、获取用户openid
		$tools = new \JsApiPay();
		...
		$input->SetOut_trade_no($info['rands']);
		$input->SetTotal_fee($tal_fee);
		$input->SetTime_start(date("YmdHis"));
		$input->SetTime_expire(date("YmdHis", time() + 600));
		$input->SetGoods_tag("test");
		$input->SetNotify_url("http://www.ineusoft.com/yuyue/index.php/Home/Wxpay/notify");
